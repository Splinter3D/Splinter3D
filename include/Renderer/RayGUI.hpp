...
// Try reading the files as text file first
    ...
    if (rgsFile != NULL)
    {
        ...
        char* fgets_result                 = fgets(buffer, MAX_LINE_BUFFER_SIZE, rgsFile);
        (void) fgets_result;
...
#ifdef _WIN32
                        sscanf_s(buffer, "f %d %s %[^\r\n]s", &fontSize, charmapFileName, (unsigned) _countof(charmapFileName), fontFileName, (unsigned) _countof(fontFileName));
#else
    ...
    if (tryBinary)
    {
        ...
        if (rgsFile != NULL)
        {
            ...
            if (fileDataSize > 0)
            {
                ...
                size_t         fread_result = fread(fileData, sizeof(unsigned char), fileDataSize, rgsFile);
                (void) fread_result;
...
char** GuiLoadIcons(const char* fileName, bool loadIconsName)
{
    ...
    if (rgiFile != NULL)
    {
        ...
        fread_result        = fread(signature, 1, 4, rgiFile);
        (void) fread_result;
        fread_result = fread(&version, sizeof(short), 1, rgiFile);
        (void) fread_result;
        fread_result = fread(&reserved, sizeof(short), 1, rgiFile);
        (void) fread_result;
        fread_result = fread(&iconCount, sizeof(short), 1, rgiFile);
        (void) fread_result;
        fread_result = fread(&iconSize, sizeof(short), 1, rgiFile);
        (void) fread_result;
            ...
            (signature[3] == ' '))
        {
            if (loadIconsName)
            {
                ...
                for (int i = 0; i < iconCount; i++)
                {
                    ...
                    fread_result    = fread(guiIconsName[i], 1, RAYGUI_ICON_MAX_NAME_LENGTH, rgiFile);
                    (void) fread_result;
                }
            }
            else
            ...
            fread_result = fread(guiIconsPtr, sizeof(unsigned int), (int) iconCount * ((int) iconSize * (int) iconSize / 32), rgiFile);
            (void) fread_result;
        }
