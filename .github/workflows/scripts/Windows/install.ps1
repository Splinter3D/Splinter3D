...
# Comprehensive msgfmt discovery and PATH setup for build.ps1 to use
Log-Action 'Performing comprehensive msgfmt discovery...'
$vcpkgRootCandidates = @(
  (Join-Path $ProjectRoot 'vcpkg'),
  (Join-Path (Split-Path $ProjectRoot -Parent) 'vcpkg')
)

# First: try to locate msgfmt in vcpkg-installed locations (checked earlier but do a thorough search here)
$msgfmtFound = $false
foreach ($root in $vcpkgRootCandidates) {
  if (-not (Test-Path $root)) { continue }
  if ($msgfmtFound) { break }

  # Try installed/<triplet>/tools
  ...
  if (Test-Path $exe) {
    ...
    Log-Milestone ("Comprehensive discovery: Found msgfmt at {0}" -f $exe)
    $msgfmtFound = $true
    break
  }

  # Try packages/<pkg>_<triplet>/tools
...
# Second: try common Chocolatey locations if not found in vcpkg
if (-not $msgfmtFound) {
  Log-Info 'msgfmt not found in vcpkg; checking Chocolatey locations...'
  $chocoCandidates = @(
    'C:\Program Files\gettext-iconv\bin',
    'C:\Program Files\gettext\bin',
    'C:\ProgramData\chocolatey\lib\gettext*\tools\*',
  ...
  foreach ($pattern in $chocoCandidates) {
    ...
    try {
      ...
      if ($found) {
        ...
        Log-Milestone ("Comprehensive discovery: Found msgfmt at {0}" -f $found.FullName)
        $msgfmtFound = $true
        break
      }
    } catch { }
...
if (-not $msgfmtFound) {
  Log-Warn 'msgfmt not found in comprehensive discovery; build.ps1 will attempt to locate it'
} else {
  # Verify msgfmt is now accessible
  if (Get-Command msgfmt -ErrorAction SilentlyContinue) {
    Log-Info ("msgfmt now accessible at: {0}" -f (Get-Command msgfmt | Select-Object -ExpandProperty Source))
  }
}
