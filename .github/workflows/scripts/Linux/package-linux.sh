...
# Try to find executable in build/bin first, then build directory, then project root
# (CMake output location can vary by generator/project configuration)
SPLINTER_EXE=""
...
if [[ -z "$SPLINTER_EXE" ]]; then
    die "Executable not found in $BUILD_DIR/bin, $BUILD_DIR, or $PROJECT_ROOT"
fi
