...
function _tests_run()
{
    ...
    local build_system
    build_system=$(_choose_build_system)
    ...
    _info "building unit tests (splinter3D_unit_tests)…"
    if ! _run_build_tool "${build_system}" "splinter3D_unit_tests"; then
        _error "unit tests compilation error" "failed to compile splinter3D_unit_tests"
    fi
    cd .. || _error "cd failed" "could not cd .."

    _info "running unit tests…"
    if ! ./splinter3D_unit_tests; then
    ...
    if [ "$(uname -s)" == 'Darwin' ]; then
        ...
        xcrun llvm-cov report ./splinter3D_unit_tests -instr-profile=unit_tests.profdata -object ./lib*.dylib -ignore-filename-regex='.*/tests/.*' -enable-name-compression > ../code_coverage.txt
        cd ..
    else
