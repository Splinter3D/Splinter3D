...
def _bootstrap(vcpkg_path: str):
    ...
    try:
        subprocess.run([bootstrap_script], check=True)
    except subprocess.CalledProcessError as e:
...
def install_vcpkg() -> str:
    ...
    if args.dry_run:
        normalized_path = _normalize_vcpkg_root(vcpkg_path)
        if reinstall:
            logger.info(f"DRY RUN: reinstall vcpkg at {normalized_path}")
        else:
            logger.info(f"DRY RUN: ensure vcpkg is available at {normalized_path}")
    ...
    if reinstall:
        ...
        if detected_vcpkg:
            ...
            logger.info(f"vcpkg is already installed at: {vcpkg_root}")
            return vcpkg_root
    ...
    executable_path = os.path.join(vcpkg_path, "vcpkg")
